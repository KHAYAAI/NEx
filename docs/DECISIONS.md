...
## D6 — Multi-hub mesh (Phase 7 evaluation outcome)

**Decided: 2026-09-17 — Defer `exo` integration; stay with `CLAUDE.md`'s "additive, not required for v0" framing.**

Phase 7 asked for a hands-on evaluation of `exo` (exo-explore/exo) for pooling compute across multiple hub units — a different problem than Phase 1's CRDT sync (compute partitioning, not state consistency). The evaluation (`infra/multi-hub-mesh/`, reproducible via `exo-evaluation.sh`) found exo's documented Linux build path genuinely works (real dashboard build, real full dependency install), but its CPU-only inference path — the one relevant to NEx's actual target hardware tiers (Decision D3: Sophon SE9/RK3588, no GPU) — is broken upstream today: `pyproject.toml`'s `mlx` source override resolves a CUDA-tagged wheel regardless of the `mlx-cpu` extra requested, which fails to import on a true CPU-only Linux install. Coordinator-only mode (`--no-worker`) avoids that specific bug but then hit a second blocker specific to the evaluation sandbox (no IPv6 support at all, and exo's zenoh transport hardcodes an IPv6-only listener bind) — disclosed as a sandbox property, not a claim about real dev-board hardware. Net: exo isn't ready to integrate today without carrying a fork of someone else's dependency pins, which cuts against the same auditability reasoning behind D4's smolagents-over-LangGraph choice. Revisit once upstream's CPU wheel resolution is fixed and real Sophon/Jetson hardware exists to test against.
