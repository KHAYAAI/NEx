...
### AT-7-1 — Multi-hub mesh: exo evaluation, reproducible **(not CI-gated, by design — `infra/multi-hub-mesh/exo-evaluation.sh`)**

**Phase:** 7 (stretch goal, `CLAUDE.md` §5 — "Evaluate `exo`... Only start this once Phases 1-6 are stable and shipping"). Phase 7 has no stated exit criteria in the plan the way Phases 1-6 do; this entry documents what was actually verified rather than gating a build that Phase 7 never asked for.

**Steps:** clone exo-explore/exo for real, build its dashboard, install its full documented Linux CPU dependency graph (`uv sync --extra mlx-cpu`), and attempt to run it both normally and in `--no-worker` (coordinator-only) mode.

**Pass condition:** none stated by the plan — this is an evaluation, not a gate. The script's own checks confirm its findings reproduce (build succeeds; a real upstream bug in exo's CPU wheel resolution reproduces with its exact error; `--no-worker` mode avoids that bug but hits a second, sandbox-specific IPv6 blocker with its exact error).

**Status:** evaluation complete, documented in `infra/multi-hub-mesh/README.md`, reran and confirmed. exo's Linux build story is real (dashboard builds, full dependency graph installs — over 150 packages). Two real, cited blockers found, hands-on rather than assumed: (1) exo's `pyproject.toml` `mlx` source override resolves a CUDA-tagged wheel unconditionally on Linux regardless of the `mlx-cpu` extra, so a genuinely CPU-only install can't import `mlx.core` at all (`undefined symbol` ABI error) — an upstream bug, not anything in this repo's control; (2) `--no-worker` mode correctly avoids that bug but then fails on this specific sandbox's total lack of IPv6 support, because exo's zenoh transport hardcodes an IPv6-only listener bind (`rust/networking/src/lib.rs`) with no override flag — disclosed as a property of this evaluation sandbox, not a claim about real dev-board hardware. No live two-node mesh demo was achievable here as a result. Recommendation (`docs/DECISIONS.md` D6): defer integration, stay with the plan's own "additive, not required for v0" framing. Deliberately not wired into CI — see `infra/multi-hub-mesh/README.md`'s "Why this isn't in CI" for why gating the pipeline on someone else's upstream bug or this sandbox's IPv6 support isn't what an acceptance test should mean.
...
- v0.9 (2026-09-17): AT-7-1 added — a hands-on, reproducible evaluation of `exo` for Phase 7's multi-hub mesh stretch goal (`infra/multi-hub-mesh/`). Found exo's Linux build/dependency story is real, but its CPU-only inference path has a real upstream bug (a CUDA-tagged wheel resolves regardless of the CPU extra requested) and its coordinator-only mode hits a second, sandbox-specific IPv6 blocker — both cited with exact errors and source locations. Decision recorded in `docs/DECISIONS.md` D6: defer integration. Deliberately not wired into CI (see the README's "Why this isn't in CI").
